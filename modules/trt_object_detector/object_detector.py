...
from modules.trt_object_detector.utils.yolov3 import TrtYOLOv3
...
class object_detector:
	...
	"tracker_mode": 0
		...
		boxes_, scores_, labels_ = self.trt_yolov3.detect(raw_frame, confidence_level)
		time_end = time.time()
		...
		# Для рисования боксов без трекера
		for i,box in enumerate(eff_boxes):
			draw_script.add_box(Box((box[0], box[1]), (box[2], box[3]),
					color=self.colors[i]))
			draw_script.add_label(Label(eff_labels[i], (box[0] + 10, box[1] + 35),
						color=self.colors[i]))
